...
class LiDARDepthSource:
    ...
    Drop-in alongside DepthEstimator: call get() each frame; if a fresh LiDAR
    frame is available it returns a depth map resized to match the video frame,
    otherwise returns None so the pipeline falls back to Depth-Anything.
    """
    ...
    def get(self, frame_bgr: np.ndarray, label: str = "iphone") -> Optional[np.ndarray]:
        ...
        depth = np.clip(depth, _DEPTH_MIN_M, _DEPTH_MAX_M)
        return depth
